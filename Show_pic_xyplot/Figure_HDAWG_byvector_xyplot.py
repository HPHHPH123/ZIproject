...
data = fc.fig_xy_convert("C:/Users/panhuih/Pictures/ZI.png")
xx = data[0]
yy = data[1]
...
def run_example(device_id):
    ...
    awg_program = textwrap.dedent(
        """\
        wave w2_x = vect(_w2x_);
        wave w2_y = vect(_w2y_);
        playWave(w2_x,w2_y);
        """
    )

    awg_program = awg_program.replace("_w2x_", ",".join([str(x) for x in waveform_2_x]))
    awg_program = awg_program.replace("_w2y_", ",".join([str(y) for y in waveform_2_y]))
