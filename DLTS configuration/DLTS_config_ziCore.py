...
def run_example(
    ...
    device = ziCore.ziDAQServer.connectDevice(device_id,"USB")
    daq = ziCore.DataAcquisitionModule(device)
    #ziCore.api_server_version_check(daq)

    daq.setDebugLevel(0)

    # Create a base configuration: Disable all available outputs, awgs, demods, scopes,...
    ziCore.disable_everything(daq, device)
    ...
    out_channel = 0
    #out_mixer_channel = ziCore.default_output_mixer_channel(props, out_channel)
    in_channel = 0
    demod_index = 0
    osc_index = 0
    demod_rate = 10e3
