...
def video_xy_convert(path):
        cap = cv.VideoCapture(path)
        while 1:
            ret, frame = cap.read()
            img = frame
            if ret:
                img_resize  = cv.resize(img, (1080, 607))
                imgGray = cv.cvtColor(img_resize, cv.COLOR_BGR2GRAY)
                imgBlur = cv.GaussianBlur(imgGray, (7, 7), 0)
                imgCanny =cv.Canny(img_resize, 0, 255)

                cv.imshow("Video", imgCanny)
                if cv.waitKey(24) & 0xFF == 27: # ESC key
                    break
            else:
                break
        ...
        csv_file_x = os.path.join(wave_dir, "wave_x_1.csv")
        csv_file_y = os.path.join(wave_dir, "wave_y_1.csv")
...
video_xy_convert("C:/Users/panhuih/Videos/Videos_test/Demo.mp4")
